...
describe('EventService', () => {
  ...
  describe('getEventsByRPGId', () => {
    ...
    it('should throw an error if RPG does not exist', async () => {
      ...
      try {
        ...
        it('should return only events from the specified RPG', async () => {
            validatorStub.validateId.returns();
            validatorStub.validatePage.returns();
            validatorStub.validateLimit.returns();
            ...
            const events = [
            { id: 1, rpgId: 1, description: 'Event1', characterId: 10, type: 100 },
            { id: 2, rpgId: 1, description: 'Event2', characterId: 11, type: 101 }
            ];
        
            repositoryStub.getEventsByRPGId.resolves(events);
        
            characterServiceStub.getCharacterById.callsFake((id) => {
                if (id === 10) return { id: 10, name: 'Char10' };
                if (id === 11) return { id: 11, name: 'Char11' };
                return null;
            });
        
            eventTypeServiceStub.convertEventType.callsFake((id) => ({ id, name: `Type${id}` }));
        
            const result = await eventService.getEventsByRPGId(1, 1, 10);
        
            expect(repositoryStub.getEventsByRPGId.calledOnceWithExactly(1, 1, 10)).to.be.true;
  ...
  describe('createEvent', () => {
    it('should throw an error when event description is missing', async () => {
      ...
      const invalidForm = { description: '', characterId: 10, typeId: 20 };
      try {
    ...
    it('should throw an error when character does not exist', async () => {
      ...
      const invalidForm = { description: faker.string.alpha(20), characterId: 999, typeId: 20 };
      try {
        await eventService.createEvent(invalidForm);
        expect.fail('Expected error was not thrown');
      } catch (error) {
    ...
    it('should throw an error when event type does not exist', async () => {
      ...
      const invalidForm = { description: faker.string.alpha(20), characterId: 10, typeId: 50 };
      try {
        await eventService.createEvent(invalidForm);
        expect.fail('Expected error was not thrown');
      } catch (error) {
    ...
    it('should create event successfully when data is valid', async () => {
      ...
      const validForm = { description: faker.string.alpha(20), characterId: 10, typeId: 20 };
      const createdEvent = {
        id: 1,
  ...
  describe('updateEvent', () => {
    ...
    it('should throw an error if event does not exist', async () => {
      ...
      try {
        await eventService.updateEvent(99, { character: 1, type: 2, description: 'Desc' });
        expect.fail('Expected error was not thrown');
      } catch (error) {
    ...
    it('should throw an error if description is invalid', async () => {
      ...
      try {
        await eventService.updateEvent(1, { character: 10, type: 20, description: 'short' });
        expect.fail('Expected error was not thrown');
      } catch (error) {
    ...
    it('should update successfully if data is valid', async () => {
      ...
      const updatedEvent = { id: 1, description: 'New valid event description', character: 10, type: 20 };
  ...
  describe('deleteEvent', () => {
    it('should throw an error if event id is invalid', async () => {
      validatorStub.validateId.throws(new Error('Event ID is required.'));
      try {
        await eventService.deleteEvent(null);
        expect.fail('Expected error was not thrown');
      } catch (error) {
        expect(error.message).to.equal('Event ID is required.');
      }
    ...
    it('should throw an error if event id is a string', async () => {
        validatorStub.validateId.throws(new Error('Event ID must be a number.'));
        try {
          await eventService.deleteEvent('test');
          expect.fail('Expected error was not thrown');
        } catch (error) {
          expect(error.message).to.equal('Event ID must be a number.');
        }
    });

    it('should throw an error if event id is negativa', async () => {
        validatorStub.validateId.throws(new Error('Event ID must be a positive number.'));
        try {
          await eventService.deleteEvent(-1);
          expect.fail('Expected error was not thrown');
        } catch (error) {
          expect(error.message).to.equal('Event ID must be a positive number.');
        }
    });
